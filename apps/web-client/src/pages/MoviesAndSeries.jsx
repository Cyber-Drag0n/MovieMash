// apps/web-client/src/pages/MoviesAndSeries.jsx
 ...
 * Положи VITE_TMDB_BEARER в apps/web-client/.env.local
 */
...
export default function MoviesAndSeries({ navigate }) {
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                ...
                setTrends(trendingMovies.slice(0, 12).map(m => ({
                    id: m?.id,
                ...
                setNewReleases(nowPlaying.slice(0, 12).map(m => ({
                    id: m?.id,
                    img: m?.poster_path ? `${IMAGE_PREFIX}${m.poster_path}` : "/example.jpg",
                ...
                try {
                    ...
                    setMustWatch(popularArr.slice(0, 8).map(m => ({
                        id: m?.id,
                        img: m?.poster_path ? `${IMAGE_PREFIX}${m.poster_path}` : "/example.jpg",
                        duration: "-",
                ...
                const groupsPromises = selectedGenres.map(g =>
                    fetchJson(`${TMDB_BASE}/discover/movie?language=${LANG}&with_genres=${g.id}&sort_by=popularity.desc&vote_count.gte=30&page=1`, BEARER)
                ...
                setNewReleasesSeries(onTheAir.slice(0, 12).map(t => ({
                    id: t?.id,
                ...
                setTrendsSeries(trendingTV.slice(0, 12).map(t => ({
                    id: t?.id,
                    img: t?.poster_path ? `${IMAGE_PREFIX}${t.poster_path}` : "/example.jpg",
                ...
                try {
                    ...
                    setMustWatchSeries(arr.slice(0, 8).map(t => ({
                        id: t?.id,
                        img: t?.poster_path ? `${IMAGE_PREFIX}${t.poster_path}` : "/example.jpg",
                        duration: "-",
                        rating: Number(t?.vote_average?.toFixed?.(1)) || 0,
                ...
                const groupsTvPromises = selectedTvGenres.map(g =>
                    fetchJson(`${TMDB_BASE}/discover/tv?language=${LANG}&with_genres=${g.id}&sort_by=popularity.desc&page=1`, BEARER)
                        .then(d => ({ genre: g, items: Array.isArray(d?.results) ? d.results.slice(0, PER_GENRE) : [] }))
                        .catch(() => ({ genre: g, items: [] }))
                );
    ...
    const onHeroLeave = () => {
        ...
        autoplayRef.current = setInterval(() => setHeroIndex(i => (i + 1) % Math.max(1, heroSlides.length || 1)), 8000);
    };
                ...
                {slice.map(({ genre, items }) => (
                    ...
                    >
                        ...
                        <div className="card-footer">
                            ...
                            <button className="cat-go" aria-label={`Открыть жанр ${genre.name}`} onClick={(e) => { e.stopPropagation(); openGenrePage("movie", genre); }}>➜</button>
                        </div>
                    </article>
                ))}
            </div>
            ...
            <div className="trends-row" role="list" style={{ justifyContent: "flex-start" }}>
                {slice.map(it => (
                    <article className="category-card new-card" key={it.id}>
                        ...
                        <span className="new-badge"><span className="new-badge-text">{it.release}</span></span>
                    </article>
                ))}
            </div>
        );
    ...
    const renderTrendsPage = (page) => {
        ...
        return (
            <div className="trends-row" role="list" style={{ justifyContent: "flex-start" }}>
                {slice.map(t => (
                    <article className="trend-card" key={t.id} role="listitem">
    ...
    function MustWatchCard({ item }) {
        ...
        const halfStar = (item.rating || 0) - fullStars >= 0.5;
            ...
            <article className="category-card top-card must-card" role="listitem">
                ...
                <span className="trend-badge right must-right" aria-hidden="true">
                    <span className="rating-row" aria-label={`Рейтинг ${item.rating || 0} из 5`}>
                        {Array.from({ length: fullStars }).map((_, i) => <img key={"f" + i} src="/star-filled.svg" alt="" className="star-icon" />)}
                        {halfStar && <img key="half" src="/star-half.svg" alt="" className="star-icon" />}
                        {Array.from({ length: emptyStars }).map((_, i) => <img key={"e" + i} src="/star-empty.svg" alt="" className="star-icon" />)}
                    </span>
        ...
        <div className="section-block" style={{ maxWidth: 1200, margin: "20px auto 60px", padding: "0 12px", position: "relative" }}>
            ...
            <div style={{ padding: "12px 0 0" }}>
                <div className="must-row" role="list" style={{ display: "flex", gap: 22, justifyContent: "flex-start" }}>
                    {mustWatch.map(m => <MustWatchCard key={m.id} item={m} />)}
                </div>
            </div>
        </div>
    );
            ...
            <div style={{ display: "flex", gap: 22, justifyContent: "center", alignItems: "flex-start", flexWrap: "wrap" }}>
                {slice.map(({ genre, items }) => (
                    ...
                    >
                        ...
                        <div className="card-footer">
                            <span className="cat-title">{genre.name}</span>
                            <button className="cat-go" aria-label={`Открыть жанр ${genre.name}`} onClick={(e) => { e.stopPropagation(); openGenrePage("tv", genre); }}>➜</button>
                        </div>
                    </article>
                ))}
            </div>
    ...
    const renderTrendsPageSeries = (page) => {
        ...
        return (
            <div className="trends-row" role="list" style={{ justifyContent: "flex-start" }}>
                {slice.map((t, idx) => (
                    <article className="trend-card" key={`s${t.id}-${idx}`} role="listitem">
                        <div className="trend-poster-wrap" aria-hidden="true">
                            <img
                                src={t.img}
                                className="trend-poster"
    ...
    const renderMustWatchSeries = () => (
        <div className="section-block" style={{ maxWidth: 1200, margin: "20px auto 60px", padding: "0 12px", position: "relative" }}>
            ...
            <div style={{ padding: "12px 0 0" }}>
                <div className="must-row" role="list" style={{ display: "flex", gap: 22, justifyContent: "flex-start" }}>
                    {mustWatchSeries.map(m => <MustWatchCardSeries key={`s${m.id}`} item={m} />)}
                </div>
            </div>
        </div>
    );
        ...
        <section className="ms-page" aria-label="Фильмы и сериалы">
            {error && (
                <div style={{
                    ...
                    background: "#b21f2d", color: "#fff", padding: "10px 16px", borderRadius: 8, zIndex: 9999,
                    boxShadow: "0 6px 18px rgba(0,0,0,0.2)", maxWidth: "90%", display: "flex", alignItems: "center", gap: 12
                }} role="alert">
                    <strong>TMDB:</strong>
                    <span style={{ whiteSpace: "pre-wrap" }}>{String(error)}</span>
                    <button onClick={() => setError(null)} style={{ marginLeft: 12, background: "transparent", color: "#fff", border: "none", cursor: "pointer" }} aria-label="Закрыть">✕</button>
                </div>
            )}
            ...
            <div className="ms-hero" onMouseEnter={onHeroEnter} onMouseLeave={onHeroLeave}>
                ...
                <div className="ms-hero-center">
                    ...
                    <div className="ms-hero-actions" role="group" aria-label="Действия с фильмом">
                        <button className="ms-btn ms-btn-play" aria-label={`Смотреть ${heroSlides[heroIndex]?.title || ""}`}>
